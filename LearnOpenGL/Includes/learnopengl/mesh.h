	...
	void Draw(Shader shader)
	{
		...
		for (unsigned int i = 0; i < textures.size(); i++)
		{
			...
			if (name == "texture_diffuse")
				...
				number = std::to_string(specularNr++); // transfer unsigned int to stream        C++ : variable++将会返回变量本身，之后再递增，而++variable则是先递增，再返回值
...
private:
	...
	void setupMesh()
	{
		...
		*/
		...
		glVertexAttribIPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void*)0);
		// vertex normals
